...
public sealed class MazeMapRenderer
{
    ...
    /// <summary>An arrow showing where someone is and which way they are facing.</summary>
    public void DrawArrow(SpriteBatch spriteBatch, Vector2 cell, float yaw, Color color)
    {
        Vector2 centre = CellCentre(cell);
        var forward = new Vector2(MathF.Sin(yaw), -MathF.Cos(yaw));
        var right = new Vector2(-forward.Y, forward.X);
        float length = CellSize * 0.42f;

        Vector2 tip = centre + forward * length;
        Vector2 left = centre - forward * length * 0.6f - right * length * 0.7f;
        Vector2 rightCorner = centre - forward * length * 0.6f + right * length * 0.7f;
        float thickness = MathF.Max(2f, CellSize * 0.12f);
        spriteBatch.DrawLine(tip, left, color, thickness);
        spriteBatch.DrawLine(tip, rightCorner, color, thickness);
        spriteBatch.DrawLine(left, rightCorner, color, thickness);
    }
#endif
